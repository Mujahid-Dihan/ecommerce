...
const form = useForm({ _method: 'PUT', title: props.post.title, blog_category_id: props.post.blog_category_id || '', content: props.post.content || '', status: props.post.status, featured_image: null });
const submit = () => { form.post(route('admin.blog.update', props.post.id), { forceFormData: true }); };
</script>
<template>
    <Head title="Edit Post | Admin" />
    <AdminLayout>
        <template #header><Link :href="route('admin.blog.index')" class="text-gray-500 hover:text-ryans transition font-medium">Blog</Link><span class="mx-2 text-gray-300">/</span><span class="text-gray-800 font-bold">Edit</span></template>
        <div class="flex items-center justify-between mb-6"><h1 class="text-2xl font-bold text-[#111]">Edit Post</h1></div>
        <div class="bg-white border border-gray-200 rounded-lg p-6 max-w-3xl">
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <div class="pt-4 border-t border-gray-100 flex justify-end"><button type="submit" :disabled="form.processing" class="px-8 py-2.5 bg-ryans hover:bg-[#5da733] text-white rounded text-[13px] font-bold transition shadow-sm disabled:opacity-50">Update</button></div>
            </form>
        </div>
    </AdminLayout>
</template>
